...
/**
 * Serves protected embedded files used by synchronized quiz questions.
 *
 ...
 */
function videoprogresscontent_quiz_pluginfile($course, $cm, $context, string $filearea, array $args,
                                              bool $forcedownload, array $options = []): bool {
    global $DB;
    if ($context->contextlevel !== CONTEXT_MODULE || $filearea !== "question") {
        return false;
    }
    ...
    if (!$DB->record_exists_sql($sql, ["itemid" => $itemid, "plugin" => "quiz", "activityid" => $cm->instance])) {
        return false;
    }
    $filename = array_pop($args);
    $filepath = '/' . ($args ? implode('/', $args) . '/' : '');
    $file = get_file_storage()->get_file(
        $context->id, "videoprogresscontent_quiz", "question", $itemid, $filepath, $filename);
