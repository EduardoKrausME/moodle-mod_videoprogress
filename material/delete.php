...
$PAGE->set_url('/mod/videoprogress/material/delete.php', ["id" => $cm->id, "materialid" => $material->id]);
$PAGE->set_title(get_string("deletematerial", "videoprogress"));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->set_context($context);

if ($confirmed) {
    require_sesskey();
    $manager->delete($material, $context);
    redirect($returnurl, get_string("materialdeleted", "videoprogress"));
}
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('mod_videoprogress/delete_resource', [
    "title" => get_string("deletematerial", "videoprogress"),
    "message" => get_string("deletematerialconfirm", "videoprogress", format_string($material->name)),
